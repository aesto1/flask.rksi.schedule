        ...
        for day in gr['days']:
            if 'lessons' in day:
                for lesson in day['lessons']:
                    if not Weekday.query.filter_by(wdid = int(day['weekday']), date = datetime.strptime(lesson['date'], "%d-%m-%Y")).first():
                        ...
                        w = Weekday(name=wdname, wdid = int(day['weekday']), date = datetime.strptime(lesson['date'], "%d-%m-%Y"))
                        db.session.add(w)
        ...
        for day in gr['days']:
            if 'lessons' in day:
                for lesson in day['lessons']:
                    ...
                    if not Schedule.query.filter_by(group=g ,lesson_name = lesson['subject'], time=int(lesson['time']), date = datetime.strptime(lesson['date'], "%d-%m-%Y")).first():
                        g = Group.query.filter_by(name=str(gr['group_name'])).first()
                        t = Teacher.query.filter_by(name=lesson['teachers'][0]['teacher_name']).first()
                        w = Weekday.query.filter_by(wdid = int(day['weekday']), date = datetime.strptime(lesson['date'], "%d-%m-%Y")).first()
                        s = Schedule(
                            group = g,
                            teacher = t,
                            weekday = w,
                            ...
                            date = datetime.strptime(lesson['date'], "%d-%m-%Y"),
